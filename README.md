...
| 异步任务 / 迁移 | 支持表结构同步、表数据同步、CSV 导出、INSERT SQL 导出；单表导出 / 单表数据同步支持源数据筛选条件和多列值替换；同步任务提供成功 / 失败 / 源行数统计，并可下载中间 INSERT SQL |
| UX | 侧边栏宽度可拖拽、SQL 编辑器/结果区比例可拖拽、尺寸持久化到 localStorage；树结构虚拟化避免大表性能问题 |

## 异步任务与数据迁移

任务入口位于顶部的"异步任务"，当前支持导出任务、表结构同步和表数据同步。任务由后台 worker 执行，任务列表会展示状态、进度、处理行数、错误信息，并在任务成功后提供产物下载。

### 部分数据导出

导出任务支持三种范围：

- **单表**：选择 database / schema / table。
- **整库**：导出指定数据库下的所有表。
- **整连接**：遍历连接下所有数据库和表。

单表导出可以选择导出格式：

- **CSV**：默认格式，多表会自动打包为 zip。
- **INSERT SQL**：生成可直接执行的 `INSERT INTO ... VALUES ...` 文件；PostgreSQL 可选择追加 `ON CONFLICT DO NOTHING`，MySQL 对应使用 `INSERT IGNORE`。

单表导出支持填写源数据筛选条件，只需要填写 `WHERE` 后面的条件片段，例如：

```sql
tenant_id = 1001 AND deleted_at IS NULL
sn IN ('WA2002614003B622382', 'WA2002614003BF9E4E2')
```

筛选条件会在后端校验：允许较长的 IN 列表，但不允许分号、SQL 注释、NUL 字符和未闭合引号，避免把筛选框变成任意 SQL 执行入口。

### 字段替换

INSERT SQL 导出和单表数据同步都支持字段替换。可以选择一个或多个列，每个列配置一份 JSON 映射，key 是源值，value 是目标值：
...
- **未匹配保留**：找不到映射时保留源值。
- **未匹配置空值**：找不到映射时按目标列处理为空值。可空列写 `NULL`；有默认值或自增列写 `DEFAULT`；否则按列类型写 Go 零值，例如数字为 `0`、布尔为 `false`、字符串为空串。

PostgreSQL 的数组、uuid、json/jsonb 会按列类型生成可执行字面量；json/jsonb 空对象会结合列默认值判断使用 `{}` 或 `[]`。
...
- 选择源连接 / 源数据库 / 源 schema。
- 可选源表；不选源表时遍历源 schema 下所有表。
- 选择目标连接 / 目标数据库 / 目标 schema。
- 可选目标表；单表同步时不选目标表则使用源表名。

目标表不存在时，会按源表结构自动创建目标表，再继续同步字段、索引等结构差异。

### 表数据同步

表数据同步是**直接同步数据**，不是先导出 SQL 再导入目标库。执行流程是：
...
单表数据同步支持和导出相同的源数据筛选条件、字段替换配置。目标表可以不选，后端会默认使用源表名同步到目标 schema。

同步任务会记录三个行数：

- `total_rows`：源端实际读取到的行数。
- `processed_rows`：目标库实际成功写入的行数。
- `failed_rows`：失败或被跳过的行数，例如唯一键冲突、`ON CONFLICT DO NOTHING` 跳过、MySQL 重复键跳过、单行插入失败等。

同步任务仍然直接写目标库，但会同时生成一份中间 INSERT SQL 产物：

- 单表同步成功后可下载 `.sql`。
- 多表同步成功后可下载 `.zip`。
- SQL 使用目标库方言、目标 schema/table、字段替换后的值，可用于审计、复盘或手动重放。
...
| `GET`  | `/api/tasks` | 异步任务列表，支持按组、连接、类型、范围、状态、关键字和时间过滤 |
| `POST` | `/api/tasks` | 新建导出 / 表结构同步 / 表数据同步任务 |
| `GET`  | `/api/tasks/:id` | 查询单个任务状态、进度、行数统计和产物信息 |
| `POST` | `/api/tasks/:id/cancel` | 取消 pending / running 任务 |
| `DELETE` | `/api/tasks/:id` | 删除任务记录及其产物目录 |
| `GET`  | `/api/tasks/:id/download` | 下载任务产物，例如 CSV、zip 或同步生成的 INSERT SQL |
...
- [x] 表数据单元格内联编辑（带权限校验）
- [x] 数据库层级切换（会话级别）
- [x] 异步导出任务（CSV / INSERT SQL / 单表筛选 / 字段替换）
- [x] 表结构同步 / 表数据同步（单表筛选、字段替换、结果统计、中间 SQL 下载）
- [ ] MariaDB / ClickHouse / SQL Server 驱动
- [ ] 组内收藏 SQL 的版本化 / 协作编辑
- [x] 审计日志（谁、什么时间、哪条 SQL）
- [ ] Docker 官方镜像 + Compose
- [x] 支持 PostgreSQL / MySQL 作为元数据库（可选）
- [ ] 2FA / SSO（OIDC）
