...
func TestGenerateCreateTablePGUsesIdentityForAutoIncrement(t *testing.T) {
	p := syncTablePair{DestSchema: "public", DestTable: "events"}
	seqDefault := "nextval('source_events_id_seq'::regclass)"
	got := generateCreateTablePG(p, []dbexec.ColumnInfo{
		{Name: "id", DataType: "bigint", Nullable: false, AutoIncrement: true, DefaultValue: &seqDefault, IsPrimary: true},
		{Name: "name", DataType: "text", Nullable: true},
	})
	want := "CREATE TABLE \"public\".\"events\" (\n  \"id\" bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL,\n  \"name\" text,\n  PRIMARY KEY (\"id\")\n)"
	if got != want {
		t.Fatalf("unexpected PostgreSQL create DDL:\nwant %s\ngot  %s", want, got)
	}
}

func TestGenerateAddColumnPGUsesIdentityForAutoIncrement(t *testing.T) {
	p := syncTablePair{DestSchema: "public", DestTable: "events"}
	seqDefault := "nextval('source_events_id_seq'::regclass)"
	got := generateAddColumn("postgres", p, dbexec.ColumnInfo{
		Name: "id", DataType: "bigint", Nullable: false, AutoIncrement: true, DefaultValue: &seqDefault,
	})
	want := "ALTER TABLE \"public\".\"events\" ADD COLUMN \"id\" bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL"
	if got != want {
		t.Fatalf("unexpected PostgreSQL add-column DDL: want %s, got %s", want, got)
	}
}
